...
/// </summary>
public class RegexMatchOperation : ValueTransformationBase
{
    /// <inheritdoc />
    public override string EnumMemberName { get; } = nameof(RegexMatchOperation);
    ...
    public override Task<TransformationResult> Apply(TransformationResult result)
    {
        try
        {
            return Task.FromResult(result.RegexMatch(OperationDetail));
        }
        catch (Exception ex)
        {
    ...
    public override string GenerateSyntax()
    {
        ...
        // if so, we can use the OperationDetail to store the regex pattern generated
        // by the builder property.

        return OperationDetail ?? string.Empty;
    }
}

/// <summary>
/// The extension methods for the <see cref="RegexMatchOperation" /> to be used with the scripting engine.
/// </summary>
public static class RegexMatchOperationExtensions
{
    /// <summary>
    ...
    public static TransformationResult RegexMatch(this TransformationResult result, string? regExPattern)
    {
        ...
        {
            return result with { Value = result.Value is null ? null : JsonSerializer.Serialize(new[] { result.Value }) };
        }
