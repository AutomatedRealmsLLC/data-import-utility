...
/// </summary>
public class InterpolateOperation : ValueTransformationBase
{
    /// <inheritdoc />
    public override string EnumMemberName { get; } = nameof(InterpolateOperation);
    ...
    /// </remarks>
    public override string? OperationDetail { get; set; } = "${0}";
    ...
    public override Task<TransformationResult> Apply(TransformationResult result)
    {
        try
        {
            return Task.FromResult(result.Interpolate(OperationDetail));
        }
        catch (Exception ex)
        {
...
/// <summary>
/// The extension methods for the <see cref="InterpolateOperation" /> to be used with the scripting engine.
/// </summary>
public static class InterpolateOperationExtensions
{
    /// <summary>
