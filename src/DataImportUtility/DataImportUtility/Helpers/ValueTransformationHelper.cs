...
public static partial class ValueTransformationHelper
{
    ...
    private static void CheckForMissingFields(this List<FieldMapping> fieldMappings, DataTable sourceDataTable, DataTable destDataTable)
    {
        ...
        var missingFieldExceptions = new List<MissingMemberException>();

        if (missingSourceFields.Any())
        {
            missingFieldExceptions.Add(new MissingMemberException($"The source table does not contain the following target fields: {string.Join(", ", missingSourceFields)}"));
        }

        if (missingTargetFields.Any())
        {
            missingFieldExceptions.Add(new MissingMemberException($"The destination table does not contain the following target fields: {string.Join(", ", missingTargetFields)}"));
        }
        ...
        {
            throw new AggregateException(missingFieldExceptions);
        }
    }
}
