...
public class ImportTableDefinition
{
    ...
    public ImportTableDefinition Clone()
    {
        ...
        forRet.FieldDescriptors = [.. FieldDescriptors.Select(x => x.Clone())];
        forRet.FieldMappings = [.. FieldMappings.Select(x => x.Clone())];
        return forRet;
    }
    ...
    {
        FieldMappings = [.. incomingFieldMappings.Select(x => x.Clone())];
    }
}
